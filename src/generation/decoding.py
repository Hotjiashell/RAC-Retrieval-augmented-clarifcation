# From https://github.com/flbbb/scope-decoding/blob/main/src/scope/example.py
...
class MixtureLogitsProcessor(UnbatchedClassifierFreeGuidanceLogitsProcessor):
    def __init__(
        ...
        if self.mixture_mode == "hard":
            self.unfaithful_tokens = []
    ...
    def __call__(self, input_ids, scores):
        ...
        if step < self.n_untouched_logits:
            if self.mixture_mode == "hard":
                self.unfaithful_tokens.append(
                    torch.zeros(logits.shape[0], dtype=int))
            return scores
        ...
        if self.mixture_mode == "soft":
            ...
            scores_processed[mask_score] = unconditional_scores[mask_score].to(
                scores_processed.dtype
            )

            self.unfaithful_tokens.append(mask_score.int())
